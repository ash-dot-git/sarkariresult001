              ...
              {table.columns.map((col, index) => (
                <th key={col.id} className="p-1 border border-gray-300 bg-gray-100">
                  <div className="flex items-center gap-1">
                    ...
                    <button type="button" onClick={() => removeColumn(index)} className="p-1 text-red-400 hover:text-[#ff0000]" aria-label={`Remove column ${index + 1}`}><Trash2 size={16} aria-hidden="true"/></button>
                  </div>
                </th>
              ))}
